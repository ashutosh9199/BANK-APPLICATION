...
class Loan(Base):
    ...
    interest_rate = Column(Numeric(precision=5, scale=2), nullable=False)  # Annual interest rate (e.g. 10.5)
...
class LoanDocument(Base):
    ...
    document_name = Column(String, nullable=False)  # e.g. "Salary Slip", "Tax Return"
...
class LoanEMISchedule(Base):
    ...
    outstanding_principal = Column(Numeric(precision=15, scale=2), nullable=False)  # remaining principal after payment
