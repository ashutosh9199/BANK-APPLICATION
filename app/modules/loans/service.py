...
from app.modules.loans.models import Loan, LoanDocument, LoanEMISchedule, LoanRepaymentHistory, LoanStatus, EMIStatus, LoanType
from app.modules.loans.repository import loan_repo
from app.modules.users.models import User
from app.modules.storage.service import storage_service
from app.modules.loans.schemas import LoanApplicationRequest
...
class LoanService:
    ...
    def apply_for_loan(self, db: Session, user: User, schema: LoanApplicationRequest) -> Loan:
        # Check KYC approved rule (cannot apply if KYC is not APPROVED)
    ...
    def _generate_amortization_schedule(self, db: Session, loan: Loan):
        ...
        for i in range(1, loan.tenure_months + 1):
            ...
            if i == loan.tenure_months:
                # Adjust final month to match remaining principal exactly
                ...
                # In case principal component is higher than remaining principal
    ...
    def repay_loan(self, db: Session, user: User, loan_id: int, amount: Decimal) -> Loan:
        ...
        # Allocate payment to EMI Schedule
        unpaid_emis = sorted(
        ...
        for emi in unpaid_emis:
            ...
            if emi.emi_amount <= remaining_payment:
                ...
                # Partial payment towards this EMI
                emi.emi_amount -= remaining_payment
                remaining_payment = Decimal("0.00")
                
        # If the loan is fully completed, mark all remaining EMIs as paid just in case of rounding discrepancies
